...
def minimize_var(df, Sums):
    ...
    try:
        ...
        exec(
            """sol = minimize(fun, x0, method='SLSQP', bounds=bnds, constraints=cons)""",
            namespace,
        )
        print(dir())
        ...
        Array[:, R] = namespace[
            "sol"
        ].x  # insert the minimized values into the column to update the array
    except ValueError:  # rare cases of overflow
        preX0 = [
            float(i) for i in preX0.split("[")[1].split("]")[0].split(",")
        ]  # obtain the values from string
        Array[:, R] = preX0
