...
def align_seqs(refseq, pe_seq_dict, tmp_dir):
    ...
    aligned = os.popen(
        "RDPHOME/AlignmentTools pairwise-knn \
               -k 1 READS TEMPLATE".replace(
            "READS", read_filename
        )
        .replace("TEMPLATE", template_filename)
        .replace("RDPHOME", RDPHOME)
    ).readlines()
...
def get_consensus(rdp_k1_alignment, count_list):
    ...
    for i in range(len(aligned_df.columns)):
        pos_df = pd.concat(
            [aligned_df.iloc[:, i], count_df.iloc[:, i]], axis=1, keys=["base", "count"]
        )
    ...
    consensus = "NNNNNNN".join(
        [region for region in consensus.split("-") if not region == ""]
    )
    return consensus
...
def consensus_loader(sample_id, refseq, pe_seq_dict, tmp_dir):
    ...
    )  # total read count of this consensus sequence
