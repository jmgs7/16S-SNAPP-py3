...
def align_seqs(refseq, pe_seq_dict, tmp_dir):
    """
    Align paired-end read sequences to a reference sequence template.

    This function takes a Refseq object, a dictionary of paired-end sequences,
    and a temporary directory. It writes the read sequences and template sequence
    to separate FASTA files, and uses an external alignment tool to align each
    read to the template in glocal mode. The function returns the alignment
    results and a list of read counts.

    Parameters
    ----------
    refseq : Refseq
        The reference sequence object containing the template sequence and read metadata.
    pe_seq_dict : dict
        A dictionary mapping read IDs to paired-end sequences.
    tmp_dir : str
        The temporary directory where intermediate FASTA files will be stored.

    Returns
    -------
    tuple
        A tuple containing:
        - aligned (list): List of alignment results from the external tool.
        - read_count_list (list): List of read counts corresponding to each read.
    """
...
def get_consensus(rdp_k1_alignment, count_list):
    """
    Calculate the consensus sequence given a list of reads and their corresponding
    counts, and RDP pairwise-knn alignment output.

    Parameters
    ----------
    rdp_k1_alignment : list
        RDP pairwise-knn alignment output.
    count_list : list
        List of counts for each read.
    ...
    Notes
    -----
    The consensus sequence is generated by finding the most frequent base at each
    position in the alignment. The common gaps are removed by replacing them with
    a fixed length (7) of 'N's.
    """
    import pandas as pd
...
def consensus_loader(sample_id, refseq, pe_seq_dict, tmp_dir):
    """
    Generate a consensus sequence from a Refseq object and a dictionary of paired end sequences.
    ...
    refseq : Refseq
        The Refseq object.
    pe_seq_dict : dict
        A dictionary of paired end sequences.
    tmp_dir : str
...
def load_consensus(sample_id, refset, pe_seq_dict, wd):
    """
    Load consensus sequences for a given sample by processing Refseq objects and
    paired end sequences in parallel.

    Parameters
    ----------
    sample_id : str
        The sample ID for which consensus sequences are to be generated.
    refset : dict
        A dictionary of Refseq objects representing reference sequences.
    pe_seq_dict : dict
        A dictionary of paired end sequences.
    wd : str
    ...
    -------
    str
        A string containing all consensus sequences in FASTA format, joined by newline characters.
    """
    all_consensus = []
